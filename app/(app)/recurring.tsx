...
export default function Recurring() {
  ...
  return (
    <ScrollView className="flex-1 bg-[#0B0B0D] px-6 pt-4">
      <View className="mb-6">
        <Text className="text-white text-3xl font-bold">Recurring</Text>
      ...
      <View className="bg-[#0F0F12] border border-neutral-800 rounded-2xl p-5 mb-6">
        <Text className="text-neutral-400 text-xs mb-1 ml-1">Description</Text>
        <TextInput
          value={description}
        ...
        <View className="flex-row mb-4">
          <TouchableOpacity
            ...
            className={`flex-1 py-3 mr-2 rounded-xl items-center ${type === "outgoing" ? "bg-red-500/20" : "bg-[#18181B]"}`}
          >
            <Text className={`font-semibold text-sm ${type === "outgoing" ? "text-red-400" : "text-neutral-400"}`}>Expense</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => setType("incoming")}
            className={`flex-1 py-3 ml-2 rounded-xl items-center ${type === "incoming" ? "bg-green-500/20" : "bg-[#18181B]"}`}
          >
        ...
        {/* Frequency */}
        <Text className="text-neutral-400 text-xs mb-2 ml-1">Frequency</Text>
          ...
          {FREQUENCIES.map((f) => (
            <TouchableOpacity
              ...
              className={`flex-1 py-3 rounded-xl items-center border ${
                frequency === f ? "bg-white border-white" : "bg-[#18181B] border-neutral-800"
              }`}
            >
        ...
        {frequency === "monthly" && (
          <View className="mb-4">
            <Text className="text-neutral-400 text-xs mb-1 ml-1">Day of Month (1-31)</Text>
            <TextInput
              value={dayOfMonth}
              ...
              maxLength={2}
              placeholderTextColor="#6B7280"
              className="bg-[#18181B] border border-neutral-800 text-white px-4 py-3 rounded-xl"
            />
          </View>
        )}
              ...
              {CATEGORIES.map((cat) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    className={`px-3 py-2 rounded-xl border ${selected ? "bg-white border-white" : "bg-[#18181B] border-neutral-800"}`}
                  >
      ...
      {/* List */}
      <Text className="text-neutral-500 text-xs uppercase tracking-widest mb-3">Active Recurring</Text>

      {items.length === 0 ? (
        <Text className="text-neutral-500 text-sm text-center py-8">No recurring expenses yet.</Text>
      ) : (
