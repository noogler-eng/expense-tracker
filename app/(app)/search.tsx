      ...
      {options.map((opt) => (
        <TouchableOpacity
          ...
          className={`px-3 py-2 rounded-xl border ${
            selected === opt ? "bg-white border-white" : "bg-[#0F0F12] border-neutral-800"
          }`}
        >
  ...
  return (
    <View className="flex-1 bg-[#0B0B0D] px-6 pt-4">
      <View className="mb-4">
        ...
        <Text className="text-neutral-400 text-sm mt-1">Find any transaction</Text>
      </View>
      ...
      <TextInput
        ...
        className="bg-[#0F0F12] border border-neutral-800 text-white px-5 py-4 rounded-2xl mb-4"
      />
      ...
      <Text className="text-neutral-500 text-xs mb-2">{filtered.length} results</Text>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          <Animatable.View animation="fadeInUp" delay={Math.min(index * 30, 300)} duration={300} className="mb-3">
            <View className="bg-[#0F0F12] border border-neutral-800 rounded-2xl px-4 py-4">
              <View className="flex-row justify-between items-start">
                <View className="flex-1 mr-3">
                  <Text className="text-white font-medium" numberOfLines={1}>{item.description}</Text>
        ...
        ListEmptyComponent={
          <View className="items-center py-12">
            <Text className="text-neutral-500 text-base">No transactions found.</Text>
          </View>
        }
      />
    </View>
