...
export default function Summary() {
  ...
  return (
    <ScrollView className="flex-1 bg-[#0B0B0D] px-6 pt-4">
      <View className="mb-6">
        <Text className="text-white text-3xl font-bold">Summary</Text>
        <Text className="text-neutral-400 text-sm mt-1">Monthly spending overview</Text>
      </View>
      ...
      <View className="flex-row items-center justify-between mb-6">
        ...
        <Text className="text-white text-xl font-bold">{MONTHS[month]} {year}</Text>
      ...
      <Animatable.View animation="fadeInUp" duration={500}>
        <View className="bg-[#0F0F12] border border-neutral-800 rounded-2xl p-5 mb-4">
          <View className="flex-row justify-between mb-4">
            <View>
              <Text className="text-neutral-500 text-xs">Incoming</Text>
              <Text className="text-green-400 text-xl font-bold mt-1">₹{cur.incoming.toFixed(0)}</Text>
            </View>
            <View className="items-end">
              <Text className="text-neutral-500 text-xs">Outgoing</Text>
          ...
          <View className="border-t border-neutral-800 pt-3">
            <Text className="text-neutral-500 text-xs">Net</Text>
      ...
      <View className="flex-row gap-3 mb-4">
        <View className="flex-1 bg-[#0F0F12] border border-neutral-800 rounded-2xl p-4">
          <Text className="text-neutral-500 text-xs">Daily Avg</Text>
          <Text className="text-white text-lg font-bold mt-1">₹{dailyAvg.toFixed(0)}</Text>
        </View>
        <View className="flex-1 bg-[#0F0F12] border border-neutral-800 rounded-2xl p-4">
          <Text className="text-neutral-500 text-xs">Highest Day</Text>
      ...
      {categories.length > 0 && (
        <Animatable.View animation="fadeInUp" delay={200} duration={500}>
          <Text className="text-white text-lg font-semibold mb-3">By Category</Text>
          <View className="bg-[#0F0F12] border border-neutral-800 rounded-2xl p-4">
            {categories.map((cat, i) => (
              <View key={cat.name} className="mb-4">
                <View className="flex-row justify-between mb-1">
                  <Text className="text-neutral-400 text-sm">{cat.name.charAt(0).toUpperCase() + cat.name.slice(1)}</Text>
                  <Text className="text-white font-semibold text-sm">₹{cat.amount.toFixed(0)}</Text>
                </View>
