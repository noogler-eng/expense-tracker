...
export default function Groups() {
 ...
 <View className="mb-6">
 <Text style={{ color: t.text }} className="text-3xl font-bold">New Group</Text>
 <TouchableOpacity onPress={() => setView("list")}>
 <Text className="text-blue-400 text-sm mt-1">← Back</Text>
 </TouchableOpacity>
 </View>

 <Text style={{ color: t.textSecondary }} className="text-xs mb-1 ml-1">Group Name</Text>
 <TextInput
 value={groupName}
 onChangeText={setGroupName}
 placeholder="e.g., Trip to Goa, Flat expenses"
 placeholderTextColor="#6B7280"
 style={inputStyle} className="px-4 py-3 rounded-xl mb-6"
 />

 <Text style={{ color: t.textMuted }} className="text-xs uppercase tracking-widest mb-3">Select Members</Text>
 {friends.length === 0 ? (
 <Text style={{ color: t.textMuted }} className="text-sm py-4">Add friends first to create a group.</Text>
 ) : (
 friends.map((f, i) => {
 ...
 <Text style={{ color: t.text }} className="font-medium flex-1">{f.firstName} {f.lastName}</Text>
 ...
 <Text style={{ color: t.text }} className="text-3xl font-bold mt-2">{selectedGroup.name}</Text>
 ...
 <Text style={{ color: t.textSecondary }} className="text-xs mt-1">{f.firstName}</Text>
 </View>
 );
 })}
 </View>
 ...
 <Text style={{ color: t.textMuted }} className="text-xs uppercase tracking-widest mb-3">Add Group Expense</Text>
 ...
 className={`flex-1 py-3 mr-2 rounded-xl items-center ${expType === "outgoing" ? "bg-red-500/20" : ""}`}
 >
 ...
 className={`flex-1 py-3 ml-2 rounded-xl items-center ${expType === "incoming" ? "bg-green-500/20" : ""}`}
 >
 ...
 className={`px-3 py-2 rounded-xl border ${expCat === cat.value ? "bg-white border-white" : " "}`}
 >
 ...
 {/* History */}
 <Text style={{ color: t.textMuted }} className="text-xs uppercase tracking-widest mb-3">Group History</Text>
 {selectedGroup.history.length === 0 ? (
 <Text style={{ color: t.textMuted }} className="text-sm text-center py-8">No expenses yet.</Text>
 ) : (
 [...selectedGroup.history].reverse().map((t, i) => (
 <View key={t.id} style={cardStyle} className="rounded-2xl rounded-2xl px-4 py-3 mb-2">
 <View className="flex-row justify-between">
 <View className="flex-1 mr-3">
 <Text style={{ color: t.text }} className="font-medium" numberOfLines={1}>{t.description}</Text>
 <Text style={{ color: t.textMuted }} className="text-xs mt-1">{new Date(t.date).toLocaleDateString()}</Text>
 </View>
 ...
 <View className="mb-6">
 <Text style={{ color: t.text }} className="text-3xl font-bold">Groups</Text>
 <Text style={{ color: t.textSecondary }} className="text-sm mt-1">Manage group expenses</Text>
 </View>
 ...
 style={cardStyle} className="rounded-2xl px-4 py-4"
 >
 <View className="flex-row justify-between items-start">
 <View className="flex-1">
 <Text style={{ color: t.text }} className="text-base font-semibold">{item.name}</Text>
 <Text style={{ color: t.textMuted }} className="text-xs mt-1">
 {item.memberIds.length} members · {getMemberNames(item.memberIds)}
 </Text>
 <Text style={{ color: t.textSecondary }} className="text-xs mt-1">
